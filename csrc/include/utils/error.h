...
concept string_like = requires(_Str str) { static_cast<std::string>(str); } ||
                      std::same_as<_Str, void>;

template <typename _Fn, typename... _Args>
concept callable = std::invocable<_Fn, _Args...> &&
                   string_like<std::invoke_result_t<_Fn, _Args...>>;
...
struct panic_handler {
  [[noreturn]]
  static auto trap(const std::string &, std::source_location, const char *)
      -> void;
...
[[noreturn, gnu::cold]]
inline auto panic(std::format_string<_Args...> fmt, _Args &&...args,
                  std::source_location loc = std::source_location::current(),
                  const char *detail = "program panicked") -> void {
  panic_handler::trap(std::format(fmt, std::forward<_Args>(args)...), loc,
                      detail);
}
...
template <typename... _Args>
  requires(sizeof...(_Args) == 0)
[[noreturn, gnu::cold]]
inline auto panic(const std::string &msg = "", _Args &&...,
                  std::source_location loc = std::source_location::current(),
                  const char *detail = "program panicked") -> void {
  panic_handler::trap(msg, loc, detail);
}

template <__detail::condition _Cond = bool, typename... _Args> struct assume {
private:
...
public:
  ...
  [[gnu::always_inline]]
  explicit assume(_Cond &&cond, std::format_string<_Args...> fmt,
                  _Args &&...args, _Src_t loc = _Src_t::current()) {
    if (cond) [[likely]]
      return;
    if constexpr (IN_DEBUG)
      std::unreachable();
    if constexpr (sizeof...(args) == 0)
      ...
      panic<_Args...>(fmt, std::forward<_Args>(args)..., loc,
                      "assumption failed");
  }

  template <__detail::callable<_Args...> _Fn>
  [[gnu::always_inline]]
  explicit assume(_Cond &&cond, _Fn &&fn, _Args &&...args,
                  _Src_t loc = _Src_t::current()) {
    if (cond) [[likely]]
      return;
    if constexpr (IN_DEBUG)
      std::unreachable();
    panic(__detail::invoke_callable(std::forward<_Fn>(fn),
                                    std::forward<_Args>(args)...),
          loc, "assumption failed");
  }
};

template <typename _Cond> explicit assume(_Cond &&) -> assume<_Cond>;
...
template <typename _Cond, typename... _Args,
          __detail::callable<_Cond, _Args...> _Fn>
